(function (win, doc) {
    ...
    */
    ...
    $button.addEventListener('click', function (event) {
        ...
        if (!$inputUsername.value) missingFields.push('Nome');
        if (!$inputEmail.value) missingFields.push('E-mail');
        if (!$message.value) missingFields.push('Mensagem');

        if (missingFields.length > 0) {
            win.alert('Por favor, preencha os seguintes campos:\n- ' + missingFields.join('\n- '));
            return;
        }
